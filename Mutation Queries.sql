...
-- A: We will need to add an expert_of_categories entity table to store relashioships between employees and expertise categories.
-- Q: What type of relationship is this? (e.g. 1-1, 1-many, or many-to-many?)
-- A: this will be many to many relashioship between employees and expert of category entities.
...
select employee_id, 4 as category_id
from employees e
where e.city = 'London';
--well, you sorta cheated by lookup up the 4 for category id
--consider this way - using a subquery as a select expression
INSERT INTO employees_categories(employee_id, category_id) (	
    SELECT employee_id, (
